...
(function ($) {
  ...
  $.fn.jQCloud = function (word_array, options) {
    ...
    var $this = this; // Namespace word ids to avoid collisions between multiple clouds

    var cloud_namespace = $this.attr('id') || Math.floor(Math.random() * 1000000).toString(36); // Default options value
    ...
    var default_options = {
      ...
      shape: false,
      // It defaults to elliptic shape
      encodeURI: true,
      removeOverflowing: true
    };
    options = $.extend(default_options, options || {}); // Add the "jqcloud" class to the container for easy CSS styling, set container width/height

    $this.addClass("jqcloud").width(options.width).height(options.height); // Container's CSS position cannot be 'static'
    ...
    var drawWordCloud = function drawWordCloud() {
      ...
      var hitTest = function hitTest(elem, other_elems) {
        ...
        var i = 0; // Check elements for overlap one by one, stop and return false as soon as an overlap is found
      ...
      var step = options.shape === "rectangular" ? 18.0 : 2.0,
          ...
          aspect_ratio = options.width / options.height; // Function to draw a word, by moving it in spiral until it finds a suitable empty place. This will be iterated on each word.
      ...
      var drawOneWord = function drawOneWord(index, word) {
        ...
        steps_in_direction = 0.0,
            ...
            custom_class = "",
            inner_html = "",
            word_span; // Extend word html options with defaults
        ...
        word_span = $('<span>').attr(word.html).addClass('w' + weight + " " + custom_class); // Append link if word.url attribute was set
        ...
        word_span.append(inner_html); // Bind handlers to words
        ...
        var width = word_span.width(),
            ...
            top = options.center.y - height / 2.0; // Save a reference to the style property, for better performance
        ...
        already_placed_words.push(word_span[0]); // Invoke callback if existing
...
var word_arrays = [{
  ...
  text: "neglected children",
  weight: 9,
  html: {
    class: "category fees"
  }
} //   {text: "child abuse", weight: 2.8, html: {class: "category fees"}},
...
jquery__WEBPACK_IMPORTED_MODULE_0___default()("#word-cloud").jQCloud(word_arrays, {
  width: 500,
  height: 350,
  afterCloudRender: function afterCloudRender() {
    jquery__WEBPACK_IMPORTED_MODULE_0___default()('#word-cloud > span').on('click', function (e) {
      e.preventDefault();
      console.log(e.target.innerHTML);
      jquery__WEBPACK_IMPORTED_MODULE_0___default()('.detailed-tags').prepend("<div class='tag'>" + e.target.innerHTML + "</div>");
    });
  }
});
